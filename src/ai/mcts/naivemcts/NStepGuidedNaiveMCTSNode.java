 ...
 * @author santi
 */
public class NStepGuidedNaiveMCTSNode extends MCTSNode {
    ...
    HashMap<BigInteger, NStepGuidedNaiveMCTSNode> childrenMap = new LinkedHashMap<BigInteger, NStepGuidedNaiveMCTSNode>();    // associates action codes with children
    ...
    //    static int count = -1;
    public boolean pruned = true;


    public NStepGuidedNaiveMCTSNode(int maxplayer, int minplayer, GameState a_gs, NStepGuidedNaiveMCTSNode a_parent, double a_evaluation_bound, int a_creation_ID, AI guide, boolean fensa) throws Exception {
        parent = a_parent;
        gs = a_gs;
    ...
    // Naive Sampling:
    public NStepGuidedNaiveMCTSNode selectLeaf(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
        ...
        */
        ...
        if (children.size() > 0 && (r.nextFloat() >= epsilon_0 || pruned)) {
            // sample from the global MAB:
            NStepGuidedNaiveMCTSNode selected = null;
    ...
    public NStepGuidedNaiveMCTSNode selectFromAlreadySampledEpsilonGreedy(float epsilon_g) throws Exception {
        if (r.nextFloat() >= epsilon_g) {
            NStepGuidedNaiveMCTSNode best = null;
            for (MCTSNode pate : children) {
                if (type == 0) {
                    // max node:
                    if (best == null || (pate.accum_evaluation / pate.visit_count) > (best.accum_evaluation / best.visit_count)) {
                        best = (NStepGuidedNaiveMCTSNode) pate;
                    }
                } else {
                    // min node:
                    if (best == null || (pate.accum_evaluation / pate.visit_count) < (best.accum_evaluation / best.visit_count)) {
            ...
            NStepGuidedNaiveMCTSNode best = (NStepGuidedNaiveMCTSNode) children.get(r.nextInt(children.size()));
            return best;
        }
    }


    public NStepGuidedNaiveMCTSNode selectFromAlreadySampledUCB1(float C) throws Exception {
        NStepGuidedNaiveMCTSNode best = null;
        double bestScore = 0;
        for (MCTSNode pate : children) {
    ...
    public NStepGuidedNaiveMCTSNode selectLeafUsingLocalMABs(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
        PlayerAction pa2;
        ...
        if (children.isEmpty()) {
            ...
            NStepGuidedNaiveMCTSNode node = new NStepGuidedNaiveMCTSNode(maxplayer, minplayer, gs2.clone(), this, evaluation_bound, a_creation_ID, guide, forceExplorationOfNonSampledActions);
            actions.add(pa2);
        ...
        NStepGuidedNaiveMCTSNode pate = childrenMap.get(actionCode);
        if (pate == null) {
            actions.add(pa2);
    ...
    public void propagateEvaluation(double evaluation, NStepGuidedNaiveMCTSNode child) {
        ...
        if (parent != null) {
            ((NStepGuidedNaiveMCTSNode) parent).propagateEvaluation(evaluation, this);
        }
    }
