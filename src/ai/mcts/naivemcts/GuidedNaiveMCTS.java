 ...
 * @author santi
 */
public class GuidedNaiveMCTS extends AIWithComputationBudget implements InterruptibleAI {
    ...
    protected GuidedNaiveMCTSNode tree = null;
    ...
    public int global_strategy = GuidedNaiveMCTSNode.E_GREEDY;
    ...
    public static double numNode = 0;
    public static double numInner = 0;
    public static int[] width = new int[25];

    public GuidedNaiveMCTSNode getRoot() {
        return tree;
    }

    public GuidedNaiveMCTS(UnitTypeTable utt) {
        this(100, -1, 100, 10,
                0.3f, 0.0f, 0.4f,
                new RandomBiasedAI(),
                new SimpleSqrtEvaluationFunction3(), new LightRush(utt, new AStarPathFinding()), true);
    }


    public GuidedNaiveMCTS(int available_time, int max_playouts, int lookahead, int max_depth,
    ...
    public GuidedNaiveMCTS(int available_time, int max_playouts, int lookahead, int max_depth, float e_l, float e_g, float e_0, AI policy, EvaluationFunction a_ef, AI gd, boolean fensa) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    public GuidedNaiveMCTS(int available_time, int max_playouts, int lookahead, int max_depth, float e_l, float e_g, float e_0, int a_global_strategy, AI policy, EvaluationFunction a_ef, AI gd, boolean fensa) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    public static double all1 = 0;
    public static int c = 0;

    public AI clone() {
        return new GuidedNaiveMCTS(TIME_BUDGET, ITERATIONS_BUDGET, MAXSIMULATIONTIME, MAX_TREE_DEPTH, epsilon_l, discount_l, epsilon_g, discount_g, epsilon_0, discount_0, playoutPolicy, ef, guide, forceExplorationOfNonSampledActions);
    }
...
//                    max = w;
//            }
//            System.out.println("G "+Arrays.toString(width));
//            System.out.println("G "+tree.children.get(getMostVisitedActionIdx()).visit_count/500.0);
//            System.out.printf("G %.2f %d",numNode/numInner, max);
//            System.out.printf("NaiveMCTS: %.2f",numNode/numInner);
//            System.out.println(" " + tree_depth);
//            System.out.printf("G %.4f %.4f %d\n", tree.children.get(getMostVisitedActionIdx()).visit_count / 500.0,
//                    numNode / numInner, tree_depth);
            numNode = 0;
            numInner = 0;
            width = new int[25];
//            MCTSUtils.treeVisulization(tree);
//            System.exit(0)
            int bestIdx = -1;
            GuidedNaiveMCTSNode best = null;
//            int visit_sum = 0;
            for (int i = 0; i < tree.children.size(); i++) {
                GuidedNaiveMCTSNode child = (GuidedNaiveMCTSNode) tree.children.get(i);
//                visit_sum+=child.visit_count;
//                System.out.println(tree.visit_count+" "+child.visit_count);
...
//            System.out.println(visit_sum);

            double sameCount = 0;
//            PlayerAction guidance;
//            if (maxplayer == 0) {
//                guidance = guide.getAction(player, gs);
//            } else {
//                guidance = guide.getAction(1 - type, gs);
//            }
//            System.out.println(guidance);
//            System.out.println(tree.actions.get(0));
//            System.out.println(tree.actions.get(bestIdx));
//            boolean flag = false;
//            PlayerAction g = guide.getAction(player, gs);
//            if (g.equals(getBestActionSoFar())) {
//                flag = true;
//            }
//            System.out.println("G " + flag);
            for (Pair<Unit, UnitAction> pair : tree.actions.get(bestIdx).getActions()) {
//                System.out.println(tree.actions.get(0).getAction(pair.m_a)+","+pair.m_b);
                if (tree.actions.get(0).getAction(pair.m_a).equals(pair.m_b)) {
                    sameCount++;
                }
            }
            all1 += (best.visit_count + 0.0) / 500;
            all += getMostVisitedActionIdx() == 0 ? 1 : 0;
            c++;
//            System.out.println("Guided: "+tree.children.size()+" "+(sameCount/tree.actions.get(bestIdx).getActions().size())+" "+ tree.moveGenerator.getSize());
//            System.out.println("Guided: " + all / c);
    ...
    public void startNewComputation(int a_player, GameState gs) throws Exception {
        player = a_player;
        current_iteration = 0;
        tree = new GuidedNaiveMCTSNode(player, 1 - player, gs, null, ef.upperBound(gs), current_iteration++, guide, forceExplorationOfNonSampledActions);
...
//            if(count>ITERATIONS_BUDGET*epsilon_f) {
//                epsilon_l = 0;
//                epsilon_g = 0;
//            }
//            System.out.println(count+" "+epsilon_l);
...
//        System.out.println("HL: " + count + " time: " + (System.currentTimeMillis() - start) + " (" + available_time + "," + max_playouts + ")");
    ...
    public boolean iteration(int player) throws Exception {
        GuidedNaiveMCTSNode leaf = tree.selectLeaf(player, 1 - player, epsilon_l, epsilon_g, epsilon_0, global_strategy, MAX_TREE_DEPTH, current_iteration++);
...
//            System.out.println(total_runs + " - " + epsilon_0 + ", " + epsilon_l + ", " + epsilon_g);
    ...
    public PlayerAction getBestActionSoFar() {
        ...
        if (DEBUG >= 1) {
            GuidedNaiveMCTSNode best = (GuidedNaiveMCTSNode) tree.children.get(idx);
...
//            if (best == null || (child.accum_evaluation/child.visit_count)>(best.accum_evaluation/best.visit_count)) {
    ...
    public GuidedNaiveMCTSNode getTree() {
        return tree;
    }
