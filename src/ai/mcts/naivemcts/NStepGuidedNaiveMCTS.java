 ...
 * @author santi
 */
public class NStepGuidedNaiveMCTS extends AIWithComputationBudget implements InterruptibleAI {
    ...
    protected NStepGuidedNaiveMCTSNode tree = null;
    ...
    public int global_strategy = NStepGuidedNaiveMCTSNode.E_GREEDY;
    ...
    public NStepGuidedNaiveMCTSNode getRoot() {
        return tree;
    }

    public NStepGuidedNaiveMCTS(UnitTypeTable utt) {
        this(100, -1, 100, 10,
                0.3f, 0.0f, 0.4f,
                new RandomBiasedAI(),
    ...
    public NStepGuidedNaiveMCTS(int available_time, int max_playouts, int lookahead, int max_depth,
    ...
    public NStepGuidedNaiveMCTS(int available_time, int max_playouts, int lookahead, int max_depth, float e_l, float e_g, float e_0, AI policy, EvaluationFunction a_ef, AI gd, boolean fensa) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    public NStepGuidedNaiveMCTS(int available_time, int max_playouts, int lookahead, int max_depth, float e_l, float e_g, float e_0, int a_global_strategy, AI policy, EvaluationFunction a_ef, AI gd, boolean fensa) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    public AI clone() {
        return new NStepGuidedNaiveMCTS(TIME_BUDGET, ITERATIONS_BUDGET, MAXSIMULATIONTIME, MAX_TREE_DEPTH, epsilon_l, discount_l, epsilon_g, discount_g, epsilon_0, discount_0, playoutPolicy, ef, guide, forceExplorationOfNonSampledActions);
    }
    ...
    public void startNewComputation(int a_player, GameState gs) throws Exception {
        player = a_player;
        current_iteration = 0;
        tree = new NStepGuidedNaiveMCTSNode(player, 1 - player, gs, null, ef.upperBound(gs), current_iteration++, guide, forceExplorationOfNonSampledActions);
...
//        System.out.println("HL: " + count + " time: " + (System.currentTimeMillis() - start) + " (" + available_time + "," + max_playouts + ")");
    ...
    public boolean iteration(int player) throws Exception {
        NStepGuidedNaiveMCTSNode leaf = tree.selectLeaf(player, 1 - player, epsilon_l, epsilon_g, epsilon_0, global_strategy, MAX_TREE_DEPTH, current_iteration++);
    ...
    public PlayerAction getBestActionSoFar() {
        ...
        if (DEBUG >= 1) {
            NStepGuidedNaiveMCTSNode best = (NStepGuidedNaiveMCTSNode) tree.children.get(idx);
    ...
    public int getMostVisitedActionIdx() {
        ...
        for (int i = 0; i < tree.children.size(); i++) {
            NStepGuidedNaiveMCTSNode child = (NStepGuidedNaiveMCTSNode) tree.children.get(i);
            if (DEBUG >= 2) {
    ...
    public void simulate(GameState gs, int time) throws Exception {
        ...
        do {
            if (gs.isComplete()) {
                ...
                gs.issue(new RandomBiasedAI().getAction(0, gs));
                gs.issue(new RandomBiasedAI().getAction(1, gs));
            }
        } while (!gameover && gs.getTime() < time);
    }

    public NStepGuidedNaiveMCTSNode getTree() {
        return tree;
    }
