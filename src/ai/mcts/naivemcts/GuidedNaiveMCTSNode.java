 ...
 * @author santi
 */
public class GuidedNaiveMCTSNode extends MCTSNode {

    public static final int E_GREEDY = 0;
    public static final int UCB1 = 1;

    static public int DEBUG = 0;
    AI guide = new LightRush(new UnitTypeTable(), new AStarPathFinding());
    ...
    HashMap<BigInteger, GuidedNaiveMCTSNode> childrenMap = new LinkedHashMap<BigInteger, GuidedNaiveMCTSNode>();    // associates action codes with children
...
//    public int ID = -1;
    ...
    public GuidedNaiveMCTSNode(int maxplayer, int minplayer, GameState a_gs, GuidedNaiveMCTSNode a_parent, double a_evaluation_bound, int a_creation_ID, AI guide, boolean fensa) throws Exception {
//        count++;
//        ID = count;
        parent = a_parent;
        gs = a_gs;
...
//        System.out.println(guide);
    ...
    // Naive Sampling:
    public GuidedNaiveMCTSNode selectLeaf(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
        ...
        */
        ...
        if (children.size() > 0 && r.nextFloat() >= epsilon_0) {
            ...
            GuidedNaiveMCTSNode selected = null;
    ...
    public GuidedNaiveMCTSNode selectFromAlreadySampledEpsilonGreedy(float epsilon_g) throws Exception {
        if (r.nextFloat() >= epsilon_g) {
            ...
            GuidedNaiveMCTSNode best = (GuidedNaiveMCTSNode) children.get(r.nextInt(children.size()));
            return best;
        }
    }


    public GuidedNaiveMCTSNode selectFromAlreadySampledUCB1(float C) throws Exception {
        GuidedNaiveMCTSNode best = null;
        double bestScore = 0;
        for (MCTSNode pate : children) {
    ...
    public GuidedNaiveMCTSNode selectLeafUsingLocalMABs(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
        PlayerAction pa2;
        BigInteger actionCode;
//        guide.reset();
//        System.out.println(guide.getClass().getSimpleName());
        if (children.isEmpty()) {
            if(children.isEmpty()) GuidedNaiveMCTS.numInner++;
            GuidedNaiveMCTS.numNode++;
            GuidedNaiveMCTS.width[this.depth+1]++;
            PlayerAction guidance;
            if (maxplayer == 0) {
                guidance = guide.getAction(type, gs);
            } else {
...
//                        System.out.println(code+" "+gua +" "+unitActionTable.get(j).actions);
            ...
            GameState gs2 = gs.cloneIssue(guidance);
            GuidedNaiveMCTSNode node = new GuidedNaiveMCTSNode(maxplayer, minplayer, gs2.clone(), this, evaluation_bound, a_creation_ID, guide, forceExplorationOfNonSampledActions);
            actions.add(pa2);
...
//        System.out.println("real:"+pa2.getActions());
        GuidedNaiveMCTSNode pate = childrenMap.get(actionCode);
        if (pate == null) {
            if(children.isEmpty()) GuidedNaiveMCTS.numInner++;
            GuidedNaiveMCTS.numNode++;
            GuidedNaiveMCTS.width[this.depth+1]++;
    ...
    public void propagateEvaluation(double evaluation, GuidedNaiveMCTSNode child) {
...
//        if (child!=null) System.out.println(evaluation);
        ...
        if (parent != null) {
            ((GuidedNaiveMCTSNode) parent).propagateEvaluation(evaluation, this);
        }
    }
