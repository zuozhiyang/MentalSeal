 ...
 * @author santi
 */
public abstract class RndAbstractionLayerAI extends AIWithComputationBudget {
    ...
    //      - harvest(target)
    //      - attack(target)
    protected HashMap<Unit, RndAbstractAction> actions = new LinkedHashMap<>();
    ...
    public RndAbstractionLayerAI(PathFinding a_pf) {
        super(-1, -1);
        pf = a_pf;
    }

    public RndAbstractionLayerAI(PathFinding a_pf, int timebudget, int cyclesbudget) {
    ...
    public PlayerAction translateActions(int player, GameState gs) {
        ...
        for (RndAbstractAction aa : actions.values()) {
    ...
    public RndAbstractAction getAbstractAction(Unit u) {
        return actions.get(u);
    }

    public void move(Unit u, int x, int y) {
        actions.put(u, new RndMove(u, x, y, pf));
    }

    public void train(Unit u, UnitType unit_type) {
        actions.put(u, new RndTrain(u, unit_type));
    }

    public void build(Unit u, UnitType unit_type, int x, int y) {
        actions.put(u, new RndBuild(u, unit_type, x, y, pf));
    }

    public void harvest(Unit u, Unit target, Unit base) {
        actions.put(u, new RndHarvest(u, target, base, pf));
    }

    public void attack(Unit u, Unit target) {
        actions.put(u, new RndAttack(u, target, pf));
    }

    public void idle(Unit u) {
        actions.put(u, new RndIdle(u));
    }
    ...
    public boolean buildIfNotAlreadyBuilding(Unit u, UnitType type, int desiredX, int desiredY, List<Integer> reservedPositions, Player p, PhysicalGameState pgs) {
        RndAbstractAction action = getAbstractAction(u);
//        System.out.println("buildIfNotAlreadyBuilding: action = " + action);
        if (!(action instanceof RndBuild) || ((RndBuild) action).type != type) {
