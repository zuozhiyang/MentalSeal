...
//            if (maxplayer == 0) {
//                guide[0].reset();
//                guidance = guide[0].getAction(type, gs);
//            } else {
//                guide[0].reset();
//                guidance = guide[0].getAction(1 - type, gs);
//            }
...
//            System.err.println("NaiveMCTSNode: This should not have happened...");
    ...
    public GuidedGreedyNaiveMCTSNode selectLeaf(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, float epsilon_s, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
        ...
        */
        if (children.size() > 0 && r.nextFloat() >= epsilon_0) {
            ...
            return selectLeafForRoot(maxplayer, minplayer, epsilon_l, epsilon_g, epsilon_0, epsilon_s, global_strategy, max_depth, a_creation_ID);
        } else {
    ...
    private GuidedGreedyNaiveMCTSNode selectLeafForRoot(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, float epsilon_s, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
        if (maxplayer == 0) {
            guidance = guide[0].getAction(type, gs);
        } else {
            guidance = guide[0].getAction(1 - type, gs);
        ...
        if (children.isEmpty() || r.nextFloat() < epsilon_s) {
...
//        int s2 = -1;
        while (!notSampledYet.isEmpty()) {
            int s = notSampledYet.remove(r.nextInt(notSampledYet.size()));
            if (unitActionTable.get(s).u.getType().canAttack) {
                s1 = s;
                break;
            }
        }
...
//        }
        for (int i = 0; i < guidance.getActions().size(); i++) {
            UnitAction gua = guidance.getActions().get(i).m_b;
            long type = guidance.getActions().get(i).m_a.getID();
            if ((s1!=-1&&unitActionTable.get(s1).u!= guidance.getActions().get(i).m_a)
//                    && (s2!=-1 && unitActionTable.get(s2).u!= guidance.getActions().get(i).m_a)
            ) {
                for (int j = 0; j < unitActionTable.size(); j++) {
                    if (type == unitActionTable.get(j).u.getID()) {
                        code = unitActionTable.get(j).actions.indexOf(gua);
                        pa2.addUnitAction(unitActionTable.get(j).u, unitActionTable.get(j).actions.get(code));
                        actionCode = actionCode.add(BigInteger.valueOf(code).multiply(multipliers[j]));
                        r2 = gua.resourceUsage(unitActionTable.get(j).u, gs.getPhysicalGameState());
                    }
                }
            }
        }
        if (s1 != -1) {
            try {
                UnitActionTableEntry ate = unitActionTable.get(s1);
                double[] distribution = distributions.get(s1);
                ...
                actionCode = actionCode.add(BigInteger.valueOf(code).multiply(multipliers[s1]));
...
//        if (s2 != -1) {
//            try {
//                UnitActionTableEntry ate = unitActionTable.get(s2);
//                double[] distribution = distributions.get(s2);
//                code = Sampler.weighted(distribution);
//                ua = ate.actions.get(code);
//                r2 = ua.resourceUsage(ate.u, gs.getPhysicalGameState());
//                if (!pa2.getResourceUsage().consistentWith(r2, gs)) {
...
//                pa2.getResourceUsage().merge(r2);
//                pa2.addUnitAction(ate.u, ua);
//                actionCode = actionCode.add(BigInteger.valueOf(code).multiply(multipliers[s2]));
//
//            } catch (Exception e) {
//                e.printStackTrace();
//            }
