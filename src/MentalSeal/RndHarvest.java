 ...
 * @author santi
 */
public class RndHarvest extends RndAbstractAction {
    Unit target;
    Unit base;
    PathFinding pf;

    public RndHarvest(Unit u, Unit a_target, Unit a_base, PathFinding a_pf) {
        super(u);
        target = a_target;
        base = a_base;
        pf = a_pf;
    ...
    public boolean equals(Object o) {
        if (!(o instanceof RndHarvest)) return false;
        RndHarvest a = (RndHarvest) o;
...
//            System.out.println("findPathToAdjacentPosition from Harvest: (" + target.getX() + "," + target.getY() + ")");
            ...
            try {
                move = pf.findPathToAdjacentPosition(unit, base.getX() + base.getY() * gs.getPhysicalGameState().getWidth(), gs, ru);
