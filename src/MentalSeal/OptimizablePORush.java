...
public class OptimizablePORush extends RndAbstractionLayerAI {
    ...
    public OptimizablePORush(UnitTypeTable a_utt) {
        this(a_utt, new AStarPathFinding());
    }

    public OptimizablePORush(UnitTypeTable a_utt, PathFinding a_pf) {
        super(a_pf);
        reset(a_utt);
    }

    public OptimizablePORush(UnitTypeTable a_utt, PathFinding a_pf, int[] arm) {
        super(a_pf);
        reset(a_utt);
        this.arm = arm;
    ...
    public PlayerAction getAction(int player, GameState gs) throws Exception {
        ...
        if (invisible_count == null) {
            invisible_count = new int[h][w];
        }
        int max = -1;

        for (int i = 0; i < invisible_count.length; i++) {
            for (int j = 0; j < invisible_count[0].length; j++) {
                if (!pogs.observable(j, i)) {
                    invisible_count[i][j]++;
                } else {
                    invisible_count[i][j] = 0;
                }
                if (max <= invisible_count[i][j] && pgs.getTerrain(j, i) == 0) {

                    if (max == invisible_count[i][j]) {
                        if (Math.abs(i - base_i) * Math.abs(i - base_i) + Math.abs(j - base_j) * Math.abs(j - base_j)
                                < Math.abs(base_i - scoutw_i) * Math.abs(base_i - scoutw_i) + Math.abs(base_j - scoutw_j) * Math.abs(base_j - scoutw_j)
                        ) {
                            scoutw_i = i;
                            scoutw_j = j;
                        }
                        if (Math.abs(i - base_i) * Math.abs(i - base_i) + Math.abs(j - base_j) * Math.abs(j - base_j)
                                > Math.abs(base_i - scout_i) * Math.abs(base_i - scout_i) + Math.abs(base_j - scout_j) * Math.abs(base_j - scout_j)
                        ) {
                            scout_i = i;
                            scout_j = j;
                        }
    ...
    @Override
    public AI clone() {
        return new OptimizablePORush(utt, pf, arm);
    }
    ...
    public void baseBehavior(Unit u, Player p, PhysicalGameState pgs) {
        ...
        if ((nworkers < numHarvestWorker && p.getResources() >= (workerType.cost + resourcesUsed))
                || (numBarracks <= nbarracks && numBase <= nbases && p.getResources() >= (workerType.cost + resourcesUsed + 10))
    ...
    protected void harvestWorkers(List<Unit> freeWorkers, Player p, PhysicalGameState pgs) {
        ...
        for (Unit u : freeWorkers) {
            ...
            } else if (u.getResources() != 0) {
                harvest(u, u, closestBase);
            } else if (u.getResources() == 0) {
                move(u, scoutw_j, scoutw_i);
            }
        }
    }
