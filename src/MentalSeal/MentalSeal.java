...
public class MentalSeal extends AIWithComputationBudget {
    UnitTypeTable utt;
    boolean started = false;
    AI ai;
    int[] arm = null;
    public MentalSeal(int timeBudget, int iterationsBudget) {
        super(timeBudget, iterationsBudget);
    }

    public MentalSeal(UnitTypeTable utt) {
        super(100, -1);
        this.utt = utt;
    }
        ...
        if (gameState.canExecuteAnyAction(i)) {
            if (!started) {
                ai = new GuidedGreedyNaiveMCTS(getTimeBudget(), -1, 100, 100 ,
                        ...
                        new AI[]{
                                new OptimizableRush(utt, new AStarPathFinding(), arm),
                        },
                        true);
                started = true;
            }
