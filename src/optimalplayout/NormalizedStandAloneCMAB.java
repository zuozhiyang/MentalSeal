 ...
 * @author santi
 */
public class NormalizedStandAloneCMAB {
    ...
    public NormalizedStandAloneCMAB(boolean fensa, int armNValues[], double k[]) throws Exception {
...
//            values[i] /= gcd;
//        }
//        System.out.println(gcd +" "+ Arrays.toString(values));
        ...
        for (int i = 0; i < values.length; i++) {
            values[i] /= gcd;
            actionCode = actionCode.add(BigInteger.valueOf(values[i]).multiply(multipliers[i]));
        }
    ...
    public void receiveReward(double evaluation, MacroArm choice) {
        if (gamma > 1e-5) {
//            System.out.println(choice.accum_evaluation);
            choice.accum_evaluation = gamma * evaluation - choice.accum_evaluation * (1 - gamma);
//            System.out.println(choice.accum_evaluation);
        }
        else {
        ...
        for (int i = 0; i < armTable.size(); i++) {
            ...
            for (int j = 0; j < kernel.length; j++) {
                ...
                if (v >= 0 && v < armTable.get(i).accum_evaluation.length) {
                    if (gamma > 1e-5)
                        armTable.get(i).accum_evaluation[v] = gamma * evaluation * kernel[j] + (1 - gamma) * armTable.get(i).accum_evaluation[v];
                    else
    ...
    public MacroArm getBestValued() {
        MacroArm best = null;
        for (MacroArm globalMABarm : globalMABarms) {
            if (best == null
                    || globalMABarm.accum_evaluation / globalMABarm.visit_count > best.accum_evaluation / best.visit_count
                    ||(globalMABarm.accum_evaluation / globalMABarm.visit_count > best.accum_evaluation / best.visit_count &&globalMABarm.visit_count == best.visit_count )) {
                best = globalMABarm;
            }
        }
        return best;
